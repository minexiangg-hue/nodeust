...
const rows=[];
for(const [owner,own] of owners){const start=performance.now();const ids=index.candidates(own);const candidates=[...ids].map(id=>index.posts.get(id)).filter(p=>p.post.ownerId!==owner);const ranked=rankMatches(own,candidates,new Date('2026-09-11T04:00:00Z'));rankRelatedPosts(own,candidates,new Set(ranked.map(r=>r.post.id)),new Date('2026-09-11T04:00:00Z'));rows.push({owner,ms:performance.now()-start,candidates:candidates.length,returned:ranked.length});}
...
writeFileSync('reports/match-discovery-release-2026-09-21/performance.json',JSON.stringify({summary,rows},null,2)+'\n');console.log(JSON.stringify(summary,null,2));
